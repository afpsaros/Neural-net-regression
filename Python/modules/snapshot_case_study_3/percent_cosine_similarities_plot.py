...
for ci in range(c):
    ...
    for j in range(ci, c):
        M_params_cosims[ci, j] = 100 * cosim(M_snaps_vecs[0][ci], M_snaps_vecs[0][j]) 

        CA_params_cosims[ci, j] = 100 * cosim(CA_snaps_vecs[0][ci], CA_snaps_vecs[0][j]) 
...
img = ax1.imshow(CA_params_cosims.round(2), cmap="autumn")
img.set_visible(False)   

ax1.axis('off')
ax1.axis('tight')

the_table = ax1.table(cellText=M_params_cosims.round(1), rowLabels = np.array(M_errors[0]).round(1), \
...
ax2.axis('off')
ax2.axis('tight')
the_table = ax2.table(cellText=CA_params_cosims.round(1), rowLabels = np.array(CA_errors[0]).round(1), \
                      colLabels = np.arange(1, c + 1),\
...
for ci in range(c):
    ...
    for j in range(ci, c):
        M_preds_cosims[ci, j] = 100 * cosim(np.array(M_preds[0][ci]).flatten(), np.array(M_preds[0][j]).flatten()) 

        CA_preds_cosims[ci, j] = 100 * cosim(np.array(CA_preds[0][ci]).flatten(), np.array(CA_preds[0][j]).flatten()) 
...
ax1.axis('off')
ax1.axis('tight')
the_table = ax1.table(cellText=M_preds_cosims.round(1), rowLabels = np.array(M_errors[0]).round(1), \
                      colLabels = np.arange(1, c + 1),\
...
ax2.axis('off')
ax2.axis('tight')
the_table = ax2.table(cellText=CA_preds_cosims.round(1), rowLabels = np.array(CA_errors[0]).round(1), \
                      colLabels = np.arange(1, c + 1),\
